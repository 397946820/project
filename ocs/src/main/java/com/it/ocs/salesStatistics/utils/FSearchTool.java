...
public class FSearchTool {
	 ...
	 * 搜索结果
	 * 
	 * @param keyWords搜索的关键字，要去掉首尾的空格
	 * @return 返回搜索到的对象
	 */

	public <E> List<E> searchTasks(String keyWords) {
		List<E> searchedTask = new ArrayList<>();
		int[] searchIndex = getSearchIndex(keyWords);
		for (int index : searchIndex) {
			if (index != -1 && index < mSearchObjs.size() * 2) {
				E e = (E) mSearchObjs.get(index / 2);
				if (e != null && !searchedTask.contains(e)) {
					searchedTask.add(e);
				}
			}
		}
