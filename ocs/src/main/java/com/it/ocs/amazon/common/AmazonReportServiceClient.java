 ...
 *
 */
public class AmazonReportServiceClient implements MarketplaceWebService {
	 ...
	 */
	public AmazonReportServiceClient(String awsAccessKeyId, String awsSecretAccessKey, String applicationName,
	...
	private HttpClient configureHttpClient(String applicationName, String applicationVersion) {
		...
		HttpConnectionParams.setConnectionTimeout(httpParams, 60*1000*10);
		HttpConnectionParams.setSoTimeout(httpParams, 60*1000*15);
	...
	private void addRequiredParameters(Map<String, String> parameters) throws SignatureException {
		...
		parameters.put("MWSAuthToken", "amzn.mws.b4b38aa7-09b1-b07b-2435-8d5022279f8d");
		parameters.put("Signature", signParameters(parameters, this.awsSecretAccessKey));
	}
	private void addRequiredParameters1(Map<String, String> parameters) throws SignatureException {
		parameters.put("AWSAccessKeyId", this.awsAccessKeyId);
		parameters.put("Version", "2010-10-01");
		parameters.put("SignatureVersion", "2");
		parameters.put("Timestamp", getFormattedTimestamp());
		parameters.put("Signature", signParameters1(parameters, this.awsSecretAccessKey));
	}
	...
	private String signParameters1(Map<String, String> parameters, String key) throws SignatureException {
	...
	public String downloadReport(GetReportRequest request) throws MarketplaceWebServiceException {
		return download(convertGetReport(request), request);
	}

	private String download(Map<String, String> parameters, GetReportRequest request) throws MarketplaceWebServiceException{
		String actionName = parameters.get("Action");
		...
		try {
			...
			int retries = 0;
				...
				try {
					/* Submit request */
					HttpResponse postResponse = httpClient.execute(method, httpContext);
					int status = postResponse.getStatusLine().getStatusCode();
					responseHeaderMetadata = getResponseHeaderMetadata(postResponse);
				    if (status == HttpStatus.SC_OK) {
				    	String amazonMD5 = postResponse.getHeaders("Content-MD5")[0].getValue();
				    	...
				    	String md5 = AmazonUtils.computeContentMD5Header(is);
						if(!amazonMD5.equals(md5)){
							log.info("数据丢失，MD5不一致");
						}else{
							String url = config.getServiceURL();
							if(url.indexOf(".jp")>-1){
								return new String(bytes, 0, readCount, "Shift_JIS");
							}
