<@FTL.admin id="operatingProfitSkuShipList" title="SKU运输方式维护" 
add_script_files=['admin/sys/operatingProfitSkuShip.js','admin/sys/date.js','admin/taximeter/common.js']>

<div data-options="region:'center',border:false">
	<table id="operatingProfitSkuShipDataGrid"
		data-options="
           url:'${FTL.X.global_domain}/operatingProfitSkuShip/findAll',
           fitColumns:true,
           columns: [
            [
                {field: 'sku', title: 'sku',sortable:true},
                {field: 'shippingType', title: '运输方式',sortable:true},
                {field: 'monthOfYear', title: '年月',sortable:true},
                {field: 'createdAt', title: '创建时间',formatter:getTime,sortable:true},
                {field: 'updatedAt', title: '更新时间',formatter:getTime,sortable:true}
                
            ]
        ],
        idField: 'id',
        singleSelect: true,
        rownumbers: true,
        pagination: true,
        pageSize: 50,
        border:false,
        fit:true,
        toolbar:'#operatingProfitSkuShipToolbar'">
	</table>
</div>

<div id="operatingProfitSkuShipToolbar">
	<div style="height:30px;">
		<form id="skuShipForm">
			<table style="float:left; min-width:1000px;">
				<tr style="min-width:1000px;">
					<td><label>sku：</label></td>
					<td>
						<input type="text" name="sku" class="easyui-textbox" style="width: 150px">
					</td>
					<td><label>年月：</label></td>
					<td>
						<input id="date" name="monthOfYear" class="easyui-datebox" data-options="
						　　buttonAlign: 'left', width: 180, panelAlign: 'right', editable: false">
					</td>
					<td>
						<a id="query" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-search'">查询</a>
						<a id="reset" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-remove'">重置</a>
					</td>
				</tr>
			</table>
		</form>
	</div>
	<hr>
	<div>
		<div>
			<a id="operatingProfitSkuShipUploadLinkbutton" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-import" plain="true">导入SKU运输方式</a>
			<a id="operatingProfitSkuShipExportLinkbutton" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-download" plain="true">下载模板</a>
		</div>
	</div>
</div>


<div id="uploadDialog" class="easyui-dialog" title="文件导入" closed="true" style="width: 450px; height: 200px; padding: 10px;" modal="true">
	<form id="uploadForm" method="post" enctype="multipart/form-data">
		<input class="easyui-filebox" name="file" id="file" data-options="prompt:'请选择文件...'" buttonText="选择文件"
			style="height: 30px; width: 350px;"> 
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" style="height: 30px" id="upload">导入</a>
	</form>
	<hr>
	<div id="messages" class="easyui-panel" style="width:410px;height:100px;color: red;"  closed="true">
		<ul id="showMessages_">
		</ul>
	</div>
</div>

<div id="operatingProfitSkuShip-buttons">
	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-save" id="operatingProfitSkuShipSaveLinkbutton">保存</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" id="operatingProfitSkuShipCancelLinkbutton">关闭</a>
</div>


<div id="operatingProfitSkuShipDialog" class="easyui-dialog" title="添加/编辑SKU运输方式" closed="true"
	style="width: 300px; height: 200px; padding: 10px;" buttons="#operatingProfitSkuShip-buttons" modal="true">
	<form id="operatingProfitSkuShipForm" method="post">
		<input type="hidden" name="entityId" id="entityId"/>
		<table align="center">
			<tbody>
				<tr>
					<td>sku：</td>
					<td>
						<input type="text" id="sku" name="sku" class="easyui-textbox" style="width: 200px;" disabled="true"/>
					</td>
				</tr>
				<tr>
					<td>运输方式：</td>
					<td>
						<select name="shippingType" class="easyui-combobox" style="width: 200px;" panelHeight="auto" editable="false" required="required">
							<option value="af">BY AF</option>
							<option value="sf">BY SF</option>
							<option value="co">BY CO</option>
						</select>
					</td>
				</tr>
			</tbody>
		</table>
	</form>
</div>


</@FTL.admin>
