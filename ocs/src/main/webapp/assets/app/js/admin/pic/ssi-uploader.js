            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                     ...
                     '<tr><td align="center" height="40">' + cutFileName(filename, ext, 15) + '</td></tr>'+
                     '<tr>'+
                     	'<td>'+
	                     	'<table border="0" cellspacing="0" cellpadding="0">'+
		                    	'<tr>'+
		                    		'<td style="height: 20px;"><input type="checkbox" /></td>'+
//		                    		'<td style="height: 20px; font-size:14px;"><a href="javascript:void(0)" onclick="edtiPicSun(9029)">编辑</a></td>'+
		                    		'<td style="height: 20px;"><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash">删除</span></button></td>'+
		                    	'</tr>'+
		                   '</table>'+
                     	'</td>'+
                 	 '</tr>' +
                ...
                beforeSend: function (xhr) {
                    ...
                    if (typeof thisS.options.beforeEachUpload === 'function') {
                        try {
                           ...
                           alert('There is an error in beforeEachUpload callback');
                            console.log(err);
                            thisS.abortAll();
                            return;
                        }
